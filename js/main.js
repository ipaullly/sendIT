...
function createParcel(){
    ...
    .then((res) => {
        ...
        if (res.status == 400){
            return res.json().then((myJson) => {
                output = `<p style="background: #a60000;color: white;text-align: center;padding: 20px;font-size: 1.3em;font-family: 'Boogaloo', cursive;" data-testid="createOrderFailResponse">${myJson.message}</p>`;
                return document.getElementById("redirectedLogIn").innerHTML = output;
            })
        }
    });
        ...
        .then((res) => {
            if (res.ok){
                return res.json().then((myJson) => {
                    ...
                    output += `
                    <li>
                        <h3 data-testid="returnedOrder">${orderList.item_name}</h3>
        ...
        .then((res) => {
            if (res.ok){
                return res.json().then((myJson) => {
                    ...
                    `;
                    let message = `<p style="background: #004e00;color: white;text-align: center;padding: 20px;font-size: 1.3em;font-family: 'Boogaloo', cursive;" data-testid="updatedDestination">${myJson.message}</p>`;
                    return document.getElementById("redirectedorderpage").innerHTML = message,
